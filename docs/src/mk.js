;(function () {
  ...
  mk.controllers.Network.prototype.Transports.socketio.init = function() {
    var url = window.gameBackendUrl || undefined;
  ...
  mk.controllers.Network.prototype._initialize = function () {
    ...
    this._transport.on('response', function (response) {
      if (response !== self.Responses.SUCCESS) {
        var c = self._callbacks['connection-error'];
        if (typeof c === 'function') {
          c(response);
        } else {
          alert('Error while connecting to the server. Code: ' + response);
        }
      } else {
        var c = self._callbacks['connection-success'];
        if (typeof c === 'function') {
          c();
        }
      }
    ...
    this._transport.on('disconnect', function () {
      var c = self._callbacks['disconnect'];
      if (typeof c === 'function') {
        c();
      } else {
